...
export default function GameScreen() {
  ...
  useEffect(() => {
    ...
    // Compute human index inline so no TDZ issues with later declarations
    const hIdx = state.players.findIndex((p) => p.type === "human");
    ...
    if (
      ...
      if (throwerIdx >= 0) {
        const fromPos = getPlayerScreenPos(throwerIdx, state.players, hIdx, lW, lH);
    ...
    if (prev.phase === "pick" && state.turnPhase === "throw" && prev.playerIdx >= 0) {
      const pickerPos = getPlayerScreenPos(prev.playerIdx, state.players, hIdx, lW, lH);
  ...
  }, [state.lastThrown, state.lastThrownByPlayerId, state.turnPhase, state.currentPlayerIndex]);
